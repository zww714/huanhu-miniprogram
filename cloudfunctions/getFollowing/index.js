...
function toPublicUser(user = {}, follow = {}) {
  ...
  return {
    id: user._id || follow.followingId,
    userId: user._id || follow.followingId,
...
exports.main = async (event = {}) => {
  try {
    ...
    const query = { followerId: userId, status: 'active' }
    ...
    for (const follow of follows) {
      try {
        const res = await db.collection('users').doc(follow.followingId).get()
