...
async function getAuthor(userId) {
  try {
    ...
    return {
      id: userId, userId,
      name: res.data?.name || '同学', avatar: res.data?.avatar || '',
      college: res.data?.college || '', major: res.data?.major || '',
      grade: res.data?.grade || '', campus: res.data?.campus || '',
...
function normalize(post, author) {
  ...
  return {
    ...
    tags: post.tags || [], images: post.images || [],
    ...
    author, authorName: author.name, canManage: false,
  }
}
...
exports.main = async (event = {}) => {
  try {
    ...
    return ok((res.data || []).map((post) => normalize(post, author)))
  } catch (err) {
    console.error('[getUserPosts]', err)
    return fail('获取 TA 的帖子失败', -2)
  }
}
